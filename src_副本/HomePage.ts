...
export class HomePage extends Laya.Script {
    ...
    private createPlayerInfo(container: Laya.Sprite): void {
        ...
        militaryRankText.text = Achievement.instance.getCurrentRankInfo().rank;
    ...
    private createGameModes(endlessSection: Laya.Sprite, saveSection: Laya.Sprite): void {
        ...
        endlessMode.on(Laya.Event.MOUSE_UP, this, () => {
            ...
            SceneManager.instance.navigateToScene("GameMain");
        });
        ...
        saveMode.on(Laya.Event.CLICK, this, () => {
            ...
            Laya.timer.frameOnce(1, this, () => {
                // 跳转到拯救模式场景
                SceneManager.instance.navigateToScene("SaveModeMain");
            });
        });
    ...
    private showAchievementPanel(): void {
        const rankInfo = Achievement.instance.getCurrentRankInfo();
    ...
    /**
     * 创建游戏模式按钮（带双层边框+图片+文字+点击效果）
     * @param imagePath 背景图路径 
     * @param text 按钮文字
     * @param enabled 是否可点击
     * @param width 按钮宽度（建议用屏幕比例计算）
     * @param height 按钮高度（建议用屏幕比例计算）
     */
    ...
    private showPlayerStats(playerEntry: LeaderboardEntry): void {
        if (!this.popupPanel) {
            return;
        }
        
        this.popupPanel.show("30天最佳战绩", (container: Laya.Sprite) => {
            // 创建分数显示
            const scoreText = new Laya.Text();
            scoreText.text = `分数：${playerEntry.score}`;
            ...
            // 创建段位显示
            const rankText = new Laya.Text();
            rankText.text = `段位：${playerEntry.rankName}`;
            ...
            // 创建超越百分比显示
            const percentileText = new Laya.Text();
            percentileText.text = `超越了${playerEntry.percentile}%的玩家`;
            percentileText.fontSize = 20;
            percentileText.color = "#4CAF50";
            percentileText.width = 300;
            percentileText.align = "center";
            percentileText.y = 130;
            container.addChild(percentileText);
        });
    }
} 