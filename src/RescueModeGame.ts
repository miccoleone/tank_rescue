...
export class RescueModeGame extends Laya.Script {
    ...
    private initFireButton(): void {
        ...
        bgImage.alpha = RescueModeGame.FIRE_BTN_NORMAL_ALPHA;
    ...
    private onFireStart(): void {
        ...
        if (bgImage) {
            bgImage.alpha = RescueModeGame.FIRE_BTN_PRESSED_ALPHA;
        }
    ...
    private showLeaderboard(): void {
        ...
        if (currentPlayerData.rankName.includes("长城")) {
            ...
            // 长城段位显示实际星级数量
            starCount = Math.floor((currentPlayerData.score - 66000) / RescueModeGame.POINTS_PER_RANK) + 1;
        } else if (currentPlayerData.rankName.includes("青铜")) {
            iconPath = "resources/moon.png";
            starCount = Math.min(4, Math.ceil(currentPlayerData.score / RescueModeGame.POINTS_PER_RANK) % 4 || 4);
        } else if (currentPlayerData.rankName.includes("白银")) {
            iconPath = "resources/star.png";
            starCount = Math.min(4, Math.ceil((currentPlayerData.score - 12000) / RescueModeGame.POINTS_PER_RANK) % 4 || 4);
        } else if (currentPlayerData.rankName.includes("黄金")) {
            iconPath = "resources/sun.png";
            starCount = Math.min(4, Math.ceil((currentPlayerData.score - 24000) / RescueModeGame.POINTS_PER_RANK) % 4 || 4);
        } else if (currentPlayerData.rankName.includes("钻石")) {
            iconPath = "resources/diamond.png";
            starCount = Math.min(4, Math.ceil((currentPlayerData.score - 36000) / RescueModeGame.POINTS_PER_RANK) % 4 || 4);
        } else if (currentPlayerData.rankName.includes("王者")) {
            iconPath = "resources/king.png";
            starCount = Math.min(4, Math.ceil((currentPlayerData.score - 48000) / RescueModeGame.POINTS_PER_RANK) % 4 || 4);
        }
